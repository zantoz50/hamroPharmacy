...
exports.requireAuth = async (req, res, next) => {
  try {
    ...
    if (!match) return res.status(401).json({ message: 'Authorization token required' });
    const token = match[1];
...
exports.requireOwnerOrAdmin = async (req, res, next) => {
  try {
    if (!req.user) return res.status(401).json({ message: 'Not authenticated' });
    const patient = await Patient.findById(req.params.id).select('owner').lean().exec();
    if (!patient) return res.status(404).json({ message: 'Resource not found' });
